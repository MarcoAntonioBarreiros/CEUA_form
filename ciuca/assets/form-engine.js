...
function applicable(item) {
  ...
  if (CFG.filterPurpose && f && f !== 'misto' && item.aplic && item.aplic !== 'todos') {
    okF = item.aplic.includes(f);
  }

  if (CFG.filterAnimal && a && item.spec && item.spec !== 'todos') {
    okA = a === 'ambos' ? true : item.spec === a;
  }
...
function snap() {
  document.querySelectorAll('.crit').forEach(block => {
    ...
    state[id] = {
      status: block.querySelector('.status').value,
      obs: block.querySelector('.obs').value
    };
  });
}
...
function render() {
  ...
  (CFG.criteria || []).forEach((cat, ci) => {
    cat.items.forEach((item, ii) => rows.push({...item, cat: cat.cat, id: 'i' + ci + '_' + ii}));
  });
  ...
  visible.forEach(item => {
    ...
    html += `
      <div class="crit" data-id="${item.id}" data-c="${esc(item.c)}" data-status="${esc(st)}">
        <div class="ctop">
          <div class="ctxt">${esc(item.t)}</div>
          <div class="tags">${tags}</div>
        </div>
        <div class="ctrl">
          <select class="status" onchange="setStatus('${item.id}', this.value)">
            ${STATUS_OPTS.map(o => `<option ${o === st ? 'selected' : ''}>${esc(o)}</option>`).join('')}
          </select>
          <input class="obs" value="${esc(obs)}" oninput="setObs('${item.id}', this.value)" placeholder="Observação (opcional)">
        </div>
      </div>`;
  });
...
function setStatus(id, value) {
  ...
  if (block) block.dataset.status = value;
  summary();
}
...
function items() {
  ...
  document.querySelectorAll('.crit').forEach(block => {
    arr.push({
      cat: previousCat(block),
      t: block.querySelector('.ctxt').textContent,
      c: block.dataset.c,
      status: block.querySelector('.status').value,
      obs: block.querySelector('.obs').value
    });
...
function stats(arr) {
  let oT=0, oOk=0, oNo=0, oNA=0, oPend=0, rT=0, rOk=0, av=0, no=0, na=0;

  arr.forEach(item => {
    if (item.status !== '—') av++;
    if (item.status === 'Não atende') no++;
  ...
  else if (oNo > 0) vct = ['naoconforme', `NÃO CONFORME — ${oNo} item(ns) obrigatório(s) não atendido(s). Regularizar antes do licenciamento.`, '#fee2e2', '#991b1b', '#fecaca'];
  ...
  return {oT,oOk,oNo,oNA,oPend,app,rT,rOk,av,total:arr.length,no,na,v:vct};
}
...
function collect() {
  const d = {
    ...
    g3: {
      ...
      'ART homologada no CRMV [CRMV — não exigida pelo CONCEA/CIUCA]': ck('art')
    },
    it: items(),
    s: stats(items())
  };
  ...
  d.meta = {
    ...
    grupo_animal: CFG.group,
    slug: CFG.slug || '',
    rn: CFG.rn || '',
    sei_destino: SEI_DESTINO,
    versao_formulario: 'CIUCA Instalações v1.1',
    origem: 'Formulários CIUCA GitHub Pages'
  };

  return d;
}

function flatPayload(d) {
  return {
    meta: d.meta,
    g1: d.g1,
    g2: d.g2,
    ...
    grupo_animal: d.meta.grupo_animal,
    slug: d.meta.slug,
    rn: d.meta.rn,

    instituicao: d.g1['Instituição'],
    cnpj: d.g1['CNPJ'],
    ciaep: d.g1['Credenciamento (CIAEP)'],
    endereco: d.g1['Endereço institucional'],
    municipio_uf: d.g1['Município / UF'],
    ceua_vinculada: d.g1['CEUA vinculada'],
    email_instalacao: d.g1['E-mail institucional da instalação'],

    nome_instalacao: d.g2['Nome / Identificação'],
    finalidade: d.g2['Finalidade'],
    situacao: d.g2['Situação'],
    localizacao_tipo: d.g2['Localização / tipo'],
    nivel_biosseguranca: d.g2['Nível de biossegurança'],
    campus: d.g2['Campus'],
    predio: d.g2['Prédio'],
    sala_setor: d.g2['Sala'],
    area_m2: d.g2['Área (m²)'],
    capacidade: d.g2['Capacidade'],
    animal_grupo: v('animalSelect'),
    animal_detalhamento: v('animalDetail'),

    coordenador: d.g3['Coordenador da instalação'],
    email_coord: d.g3['E-mail (Coord.)'],
    rt: d.g3['Responsável Técnico (Méd. Vet.)'],
    crmv: d.g3['CRMV'],
    crmv_uf: d.g3['UF do CRMV'],
    email_rt: d.g3['E-mail (RT)'],

    situacao_geral: d.s.v[1]
  };
}
...
function showStatus(type, msg) {
  ...
  box.style.background = type === 'error' ? '#fee2e2' : type === 'success' ? '#dcfce7' : '#fef3c7';
  box.style.color = type === 'error' ? '#991b1b' : type === 'success' ? '#166534' : '#92400e';
  box.innerHTML = (type === 'success' ? '✅ ' : type === 'error' ? '❌ ' : '⚠️ ') + esc(msg);
}
...
function pdf() {
  ...
  const kv = (k, val) => {
    ...
    doc.setFont('helvetica','bold');
    doc.setFontSize(7.4);
    const kl = doc.splitTextToSize(k + ':', 50);
    doc.text(kl, ML, y);

    doc.setTextColor(30,41,59);
    doc.setFont('helvetica','normal');
    const vl = doc.splitTextToSize(String(val || '—'), 128);
    doc.text(vl, ML+52, y);
  ...
  Object.entries(d.g1).forEach(([k,val]) => kv(k,val));

  sec('2. DADOS DA INSTALAÇÃO');
  Object.entries(d.g2).forEach(([k,val]) => kv(k,val));

  sec('3. COORDENADOR E RESPONSÁVEL TÉCNICO');
  Object.entries(d.g3).forEach(([k,val]) => kv(k,val));
  ...
  d.it.forEach(item => {
    ...
    const lines = doc.splitTextToSize(item.t, CW-32);
    ...
    doc.setTextColor(100,116,139);
    doc.setFont('helvetica','bold');
    doc.text(item.c + ' · ' + (item.status === '—' ? 'não avaliado' : item.status), MR-2, y, {align:'right'});

    y += lines.length * 3.5 + 3;

    if (item.obs) {
      const obsLines = doc.splitTextToSize('Obs.: ' + item.obs, CW-10);
      ckpg(obsLines.length * 3.5 + 2);
      doc.setTextColor(71,85,105);
      doc.setFont('helvetica','italic');
      doc.text(obsLines, ML+5, y);
  ...
  kv('Obrigatórios atendidos', d.s.oOk + ' de ' + d.s.app + ' aplicáveis; pendentes: ' + d.s.oPend);
  kv('Recomendados atendidos', d.s.rOk + ' de ' + d.s.rT);
  kv('Situação geral', d.s.v[1]);

  sec('TRAMITAÇÃO NO SEI');
  kv('Destino SEI', SEI_DESTINO);
  kv('Orientação', 'Abrir processo SEI e encaminhar para UFPR / R / PL / CEUA com este PDF assinado pelo coordenador da instalação e pelo responsável técnico, acompanhado de despacho solicitando avaliação da CEUA e encaminhamento para cadastro/regularização no CIUCA.');
  ...
  doc.setFont('helvetica','bold');
  doc.setFontSize(8);
  doc.text('ASSINATURA DO COORDENADOR DA INSTALAÇÃO:', ML, y);
  doc.line(ML+65, y+1, MR, y+1);
  y += 12;
  doc.text('ASSINATURA DO RESPONSÁVEL TÉCNICO:', ML, y);
  doc.line(ML+58, y+1, MR, y+1);
...
async function registrarDados() {
  const d = collect();
  const payload = flatPayload(d);

  showStatus('warn', 'Registrando dados na planilha interna da CEUA/UNIBIO...');

  try {
    await fetch(CIUCA_APPS_SCRIPT_URL, {
      method: 'POST',
      mode: 'no-cors',
      headers: {'Content-Type': 'text/plain;charset=utf-8'},
      body: JSON.stringify(payload)
    });
    showStatus('success', 'Dados registrados na planilha interna. Gere/anexe o PDF assinado no processo SEI.');
    return true;
  } catch (err) {
    showStatus('error', 'Erro ao registrar dados: ' + err.message);
    return false;
  }
}

async function registrarEpdf() {
  await registrarDados();
  pdf();
}
